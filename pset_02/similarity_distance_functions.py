...
"""
def find_distance(vector_filename):

    # Read in the vectors dataframe
    vectors_df = pandas.read_csv(vector_filename, index_col=0)

    # Extract my personal vector
    my_vec = vectors_df[vectors_df.index == my_hash()].values[0]

    # Define a distance function
    def my_distance(vec):
        return 1 - cosine_similarity(vec, my_vec)

    distances = vectors_df[vectors_df.index != my_hash()].apply(my_distance, axis=1)
